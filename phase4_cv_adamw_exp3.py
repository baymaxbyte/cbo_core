...
class CvAdamW(torch.optim.Optimizer):
    ...
    def get_momentum(self):
        """Return the current thermodynamic momentum (mu_t)."""
        return self._thermo_state['momentum_cv']
    ...
    def step(self, current_cv=None, train_accuracy=0.0, closure=None):
        ...
        if current_cv is not None:
            ...
            if train_accuracy >= 0.99:
                ...
                # Gate closed: momentum freezes, use last known wd
                dynamic_wd = ts['dynamic_wd']

            # ALWAYS update prev_cv (outside the gate)
            ts['prev_cv'] = current_cv
        else:
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch+1) % 100 == 0:
            print(f"  Epoch {epoch+1} | Loss:{avg_loss:.4f} TrAcc:{train_acc:.4f} "
                  f"VAcc:{val_acc:.4f} Cv:{avg_cv:.4f} WD:{dynamic_wd:.4f} μ:{momentum:.6f}")
    ...
    # Save
    with open(f"data/phase4b_exp3_metrics_seed{seed}.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=rows[0].keys())
        writer.writeheader()
        writer.writerows(rows)

    results = {"seed": seed, "grokking_epoch": grok_epoch, "kappa": args.kappa, "tau": args.tau,
               "alpha": args.alpha, "base_wd": args.base_wd,
               "max_dynamic_wd": max(r["dynamic_wd"] for r in rows)}
    with open(f"data/phase4b_exp3_results_seed{seed}.json", "w") as f:
    ...
    plt.savefig(f"figures/phase4b_exp3_cv_adamw_seed{seed}.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: figures/phase4b_exp3_cv_adamw_seed{seed}.png")
