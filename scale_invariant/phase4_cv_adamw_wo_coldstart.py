...
class CvAdamW(torch.optim.Optimizer):
    ...
    def step(self, current_cv=None, train_accuracy=0.0, closure=None):
        ...
        if current_cv is not None:
            ...
            # 1. CONTINUOUS SENSOR: Always track the distribution from Epoch 1
            mu_prev = ts['mu']
            mu_t = beta_z * mu_prev + (1 - beta_z) * v_t
            ts['mu'] = mu_t

            # EMA of variance (Welford-style)
            ...
            # 2. GATED ACTUATOR: Only inject heat after reaching equilibrium
            if train_accuracy >= 0.99:
                ts['dynamic_wd'] = base_wd + max(0.0, z_t - z_thresh)
            else:
                # Keep weight decay at baseline while network memorizes
...
def main():
    ...
    # Save
    csv_path = f"data/phase4b_wocs_seed{args.seed}_metrics.csv"
    ...
    json_path = f"data/phase4b_wocs_seed{args.seed}_results.json"
    ...
    plt.tight_layout()
    fig_path = f"figures/phase4b_wocs_seed{args.seed}_cv_adamw.png"
