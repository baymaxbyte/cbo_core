...
class ThermodynamicBenchmarker:
    """
    Evaluates multiple trigger mechanisms simultaneously without affecting training.
    Logs the exact epoch each method would have fired.
    
    Includes ghost triggers for both Phase 4B variants:
      - ema_zscore_continuous: EMA z-score tracking from epoch 1 (wo_coldstart)
      - ema_zscore_gated: EMA z-score tracking only after train_acc >= 0.99 (cold-start)
    """

    def __init__(self, static_tau=0.015, z_score_threshold=3.0, warmup_epochs=15,
                 beta_z=0.9, ema_z_thresh=2.0):
        self.warmup_epochs = warmup_epochs
        self.static_tau = static_tau
        self.z_threshold = z_score_threshold
        self.beta_z = beta_z
        self.ema_z_thresh = ema_z_thresh
        ...
        # EMA state for continuous sensor (wo_coldstart)
        self.ema_cont_mu = 0.0
        self.ema_cont_sigma_sq = 0.0

        # EMA state for gated sensor (cold-start)
        self.ema_gated_mu = 0.0
        self.ema_gated_sigma_sq = 0.0
        ...
        self.triggers = {
            ...
            "z_score": None,
            "kinematic": None,
            "ema_zscore_continuous": None,
            "ema_zscore_gated": None,
        }
    ...
    def step(self, epoch, current_cv, train_acc=0.0):
        ...
        if self.prev_cv is None:
            self.prev_cv = current_cv
            self.logs["cv"].append(current_cv)
            self.logs["velocity"].append(0.0)
            self.logs["acceleration"].append(0.0)
            self.logs["ema_cont_z"].append(0.0)
            self.logs["ema_gated_z"].append(0.0)
            return

        # 1. Calculate kinematics
        velocity = current_cv - self.prev_cv
        ...
        # --- EMA Z-SCORE (Continuous Sensor / wo_coldstart variant) ---
        mu_prev_cont = self.ema_cont_mu
        self.ema_cont_mu = self.beta_z * mu_prev_cont + (1 - self.beta_z) * velocity
        sigma_sq_cont = self.beta_z * self.ema_cont_sigma_sq + (1 - self.beta_z) * (velocity - mu_prev_cont) * (velocity - self.ema_cont_mu)
        self.ema_cont_sigma_sq = max(0.0, sigma_sq_cont)
        sigma_cont = math.sqrt(self.ema_cont_sigma_sq + eps)
        z_cont = (velocity - self.ema_cont_mu) / sigma_cont
        self.logs["ema_cont_z"].append(z_cont)

        # Continuous sensor trigger (gated actuator: only fires after train_acc >= 0.99)
        if self.triggers["ema_zscore_continuous"] is None and epoch > self.warmup_epochs:
            if train_acc >= 0.99 and z_cont > self.ema_z_thresh:
                self.triggers["ema_zscore_continuous"] = epoch
                print(f"    [EMA-ZSCORE-CONTINUOUS] Triggered at epoch {epoch} (z={z_cont:.3f} > {self.ema_z_thresh})")

        # --- EMA Z-SCORE (Gated / cold-start variant) ---
        z_gated = 0.0
        if train_acc >= 0.99:
            mu_prev_gated = self.ema_gated_mu
            self.ema_gated_mu = self.beta_z * mu_prev_gated + (1 - self.beta_z) * velocity
            sigma_sq_gated = self.beta_z * self.ema_gated_sigma_sq + (1 - self.beta_z) * (velocity - mu_prev_gated) * (velocity - self.ema_gated_mu)
            self.ema_gated_sigma_sq = max(0.0, sigma_sq_gated)
            sigma_gated = math.sqrt(self.ema_gated_sigma_sq + eps)
            z_gated = (velocity - self.ema_gated_mu) / sigma_gated

            if self.triggers["ema_zscore_gated"] is None and epoch > self.warmup_epochs:
                if z_gated > self.ema_z_thresh:
                    self.triggers["ema_zscore_gated"] = epoch
                    print(f"    [EMA-ZSCORE-GATED] Triggered at epoch {epoch} (z={z_gated:.3f} > {self.ema_z_thresh})")
        self.logs["ema_gated_z"].append(z_gated)

        # 2. Evaluate original triggers (only after warmup to avoid initialization noise)
...
def train_phase3(args):
    ...
    # Dataset
    train_ds, val_ds = create_dataset(p, seed=seed)
    ...
    for epoch in range(args.epochs):
        ...
        # Feed Cv to benchmarker (ghost triggers) — after validation so train_acc is available
        benchmarker.step(epoch, avg_cv, train_acc=train_acc)
    ...
    # Save results
    csv_path = f"data/phase3_seed{seed}_metrics.csv"
    ...
    results = {
        ...
        "config": {"p": p, "epochs": args.epochs, "tau": args.tau,
                   "z_threshold": args.z_threshold, "warmup": args.warmup, "seed": seed},
    }
    json_path = f"data/phase3_seed{seed}_benchmark_results.json"
    ...
    print(f"  Z-Score (batch) triggered:   epoch {benchmarker.triggers['z_score']}")
    print(f"  Kinematic triggered:         epoch {benchmarker.triggers['kinematic']}")
    print(f"  EMA Z-Score (continuous):    epoch {benchmarker.triggers['ema_zscore_continuous']}")
    print(f"  EMA Z-Score (gated/cold):    epoch {benchmarker.triggers['ema_zscore_gated']}")
    ...
    if grok_epoch and benchmarker.triggers["ema_zscore_continuous"]:
        lag = grok_epoch - benchmarker.triggers["ema_zscore_continuous"]
        print(f"  LAG (ema_cont -> grokking):  {lag} epochs")
    if grok_epoch and benchmarker.triggers["ema_zscore_gated"]:
        lag = grok_epoch - benchmarker.triggers["ema_zscore_gated"]
        print(f"  LAG (ema_gated -> grokking): {lag} epochs")
    print(f"{'='*60}")
    ...
    print(f"  Saved: figures/phase3_seed{seed}_triggers.png")
    print(f"  Saved: figures/phase3_seed{seed}_kinematics.png")
...
def plot_phase3(rows, benchmarker, grok_epoch, seed=42):
    ...
    colors = {"profiler": "orange", "z_score": "purple", "kinematic": "green",
              "ema_zscore_continuous": "cyan", "ema_zscore_gated": "magenta"}
    labels = {"profiler": "profiler", "z_score": "z_score (batch)",
              "kinematic": "kinematic", "ema_zscore_continuous": "EMA-Z (continuous)",
              "ema_zscore_gated": "EMA-Z (gated)"}
    for method, ep in benchmarker.triggers.items():
        if ep is not None:
            ax1.axvline(x=ep, color=colors[method], ls="--", lw=2, alpha=0.8,
                        label=f"{labels[method]}: epoch {ep}")
    if grok_epoch:
        ax1.axvline(x=grok_epoch, color="black", ls="-", lw=2, alpha=0.8,
                    label=f"Grokking: epoch {grok_epoch}")

    fig.suptitle(f"Phase 3: Trigger Benchmarking (seed={seed}) — When Would Each Method Fire?", fontsize=13)
    lines1, l1 = ax1.get_legend_handles_labels()
    lines2, l2 = ax2.get_legend_handles_labels()
    fig.legend(lines1 + lines2, l1 + l2, loc="lower center", ncol=3,
               bbox_to_anchor=(0.5, -0.05), fontsize=9)
    plt.tight_layout(rect=[0, 0.08, 1, 1])
    plt.savefig(f"figures/phase3_seed{seed}_triggers.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    fig.suptitle(f"Phase 3: Thermodynamic Kinematics (seed={seed})", fontsize=13)
    plt.tight_layout()
    plt.savefig(f"figures/phase3_seed{seed}_kinematics.png", dpi=150, bbox_inches="tight")
    plt.close()
