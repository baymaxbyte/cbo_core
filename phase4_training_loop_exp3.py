...
class KinematicTracker:
    """Detects the Cv peak crest using EMA-smoothed second-derivative analysis."""

    def __init__(self, warmup=15, alpha=0.9):
        self.warmup = warmup
    ...
    def step(self, epoch, cv, train_accuracy=0.0):
        ...
        self.ema_cv = (self.alpha * self.ema_cv) + ((1 - self.alpha) * cv)
        ...
        # 3. MEMORIZATION GATE: only trigger after model has memorized
        if not self.triggered and epoch > self.warmup and train_accuracy >= 0.99:
...
def main():
    ...
    print(f"  p={args.p}, epochs={args.epochs}, seed={seed}, device={device}\n")
    ...
    for epoch in range(args.epochs):
        ...
        # Check kinematic trigger (WITH MEMORIZATION GATE)
        if not is_heating and trigger_epoch == -1:
            peak_detected = tracker.step(epoch, avg_cv, train_acc)
            if peak_detected:
    ...
    # Save
    with open(f"data/phase4a_exp3_metrics_seed{seed}.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=rows[0].keys())
        writer.writeheader()
        writer.writerows(rows)

    results = {"seed": seed, "trigger_epoch": trigger_epoch, "grokking_epoch": grok_epoch,
               "cooldown": args.cooldown, "spike_wd": args.spike_wd,
               "lag": (grok_epoch - trigger_epoch) if grok_epoch and trigger_epoch >= 0 else None}
    with open(f"data/phase4a_exp3_results_seed{seed}.json", "w") as f:
    ...
    plt.savefig(f"figures/phase4a_exp3_intervention_seed{seed}.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: figures/phase4a_exp3_intervention_seed{seed}.png")
